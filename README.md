...
| 유저별 params / 수동밴드 / DB 암호화 키(마스킹·보기) | 거래소 잔고 실시간 반영 |
...
| USDT는 `$`, 원 환산은 `₩` | |
| `ai_level=off`면 LLM 토큰 0 (채팅·분석 409, 뉴스도 안 받음, 오토파일럿은 규칙 엔진으로 전환) | 토큰 사용량 원화 집계 |
| 자동매매: Binance 키만 있으면 실행. 검증 표 안에서 종목·주기만 고르고 밴드는 ATR로 서버가 계산, 재분석/중지 | 실주문 연동 (지금은 밴드 의도까지) |
| 폰 대응 반응형 (한 칸 레이아웃, 스티키 탭, 44px 터치 타깃, 입력 16px로 iOS 확대 방지, 차트 높이 자동) | 네이티브 앱 패키징 |
...
5. 각자 설정에서 닉네임, 알림, MIN 잔고, Binance 키, (선택) LLM 키
6. AI 키 없이 쓰려면 투자 탭 왼쪽에서 종목 검색 후 상한/하한/스케줄만 저장
7. 자동매매는 Binance 키만 있으면 투자 탭 오른쪽 카드에서 바로 실행된다. LLM 키를 넣고 설정에서 AI 개입을 "파라미터 + 종목 추천"으로 바꾸면 종목 선정이 규칙 엔진에서 AI로 넘어간다.

### 17.2.2 폰에서 쓰기

같은 주소를 폰 브라우저로 열면 된다. 별도 앱은 없다.

- 640px 이하에서 모든 그리드가 한 칸으로 접히고, 탭 줄은 위에 고정되어 가로 스크롤된다.
- 입력창 글자는 16px이라 iOS에서 초점 잡을 때 화면이 확대되지 않는다. 버튼은 최소 44px로 잡는다.
- 차트 높이는 폭의 62%로 따라오고(240~420px), 하단은 아이폰 홈바 영역만큼 여백을 둔다.
- 폰에서 열려면 API를 `HELM_HOST=0.0.0.0`으로 띄우고 같은 와이파이에서 `http://<PC IP>:8090`으로 접속한다.
...
2. 클램프 범위 안 파라미터 제안
3. `ai_level=params_and_symbols`일 때 스크리너 통과 종목 중 추천 순위와 실행 주기
...
- 손절 제거
- 스크리너 미통과 종목 강제 편입
- **가격·손절·익절·주문 금액 결정** (전부 ATR과 사용자 리스크 설정으로 서버가 계산)

### 7.0 토큰 스위치

`ai_level=off`면 LLM 토큰을 한 푼도 쓰지 않는다. 게이트는 키를 읽기 전에, 네트워크를 타기 전에 걸린다.

| 엔드포인트 | `ai_level=off` | `params_only` | `params_and_symbols` |
| --- | --- | --- | --- |
| `POST /api/ai/chat` | 409, 대화 기록도 남지 않음 | 됨 | 됨 |
| `POST /api/ai/analyze` | 409 | 됨 | 됨 |
| `POST /api/ai/autopilot/run` | 규칙 엔진, 토큰 0 | 규칙 엔진, 토큰 0 | LLM 키가 있으면 AI, 없으면 규칙 |
| `GET /api/ai/news` | `[]` (RSS도 안 받음) | 됨 | 됨 |
| `GET /api/ai/autopilot`, `POST /api/ai/autopilot/stop` | 토큰 안 씀. 항상 됨 | 됨 | 됨 |

UI도 같이 잠긴다. AI가 꺼져 있으면 투자 탭은 질문창·분석 버튼을 감추고 `messages`/`news`를 아예 호출하지 않는다. 중지는 AI를 끈 뒤에도 눌린다. 자동매매 카드에는 지금 어느 쪽이 종목을 고르는지(`AI 선정` / `규칙 선정 · 토큰 0`) 배지로 표시한다.

### 7.0.1 자동매매 (검증 우선, Binance 키만 필요)

**필요한 키는 Binance API key/secret 하나뿐이다.** LLM 키는 선택이다.

| 조건 | 종목 선정 | 토큰 |
| --- | --- | --- |
| Binance 키 없음 | 실행 거부 (400) | 0 |
| Binance 키만 있음 | 규칙 엔진: 거래대금·ADX·ATR·펀딩 점수 (전략 모드별 가중치) | 0 |
| Binance + LLM 키 + `ai_level=params_and_symbols` | LLM이 검증 표 안에서 선택 | 실행당 1회 |

돈이 걸린 판단이라 모델이 만든 숫자는 하나도 쓰지 않는다. 순서는 이렇다.

```
1. 24h 티커 → 거래대금 상위 60개, 블랙리스트·스테이블 제외
2. 거래대금 20M USDT 미만 탈락
3. 종목별 실제 봉 240개 → ATR, ADX, 최근 고저
4. ATR%가 0.15~15% 밖이면 탈락 (손절이 자주 맞거나 엣지가 없음)
5. 펀딩 APR + 뉴스 헤드라인 12개 첨부
6. AI 모드면 이 "검증된 후보 표"로 LLM 1회 호출, 규칙 모드면 표를 그대로 점수화
7. 검증기: 표에 없는 심볼 / 확신 0.55 미만 / 근거 문장 없음 / 중복 → 기각
8. 밴드 = 최근가 ± ATR x atr_stop_mult (목표 2R), 금액 = 사용자 설정값
9. 동시 포지션 한도까지만 자동매매 잡 생성 (`source=ai`), 활성 종목 교체
```

- **재분석**: 직전 3회에 돌린 종목을 후보에서 빼고 다시 고른다. 새 잡이 이전 AI 잡을 대체한다.
- **중지**: AI 잡을 끄고 `soft_stop`. 보유분 청산은 현황 탭의 전량 청산(하드킬, 5초 이중 확인).
- 통과한 추천이 없으면 아무것도 실행하지 않고 기각 사유를 보고서에 남긴다.
- 모든 실행은 `reports`(계획 마크다운)와 `ai_runs`(종목·레짐 이력)에 감사 기록이 남는다.
